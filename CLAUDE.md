...
## ⚠️ 중요 규칙
- **`style.css` 를 수정하면 전 페이지의 `style.css?v=` 숫자를 올려야 반영된다.** (현재 **v3**)
...
- 메뉴를 바꾸면 `index/status/teachers/centers` 4개 + **`t/` 750개**를 전부 고쳐야 한다.
  750개는 `tools/gen_teachers.ps1` 안의 nav 블록을 고치고 다시 돌리면 된다.
