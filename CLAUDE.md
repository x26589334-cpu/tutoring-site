...
## ⚠️ 중요 규칙
- **`style.css` 를 수정하면 전 페이지의 `style.css?v=` 숫자를 올려야 반영된다.** (현재 **v4**)
  ...
  ```
  `t/` 750개와 `c/` 195개도 함께 바뀌므로 `-r` 로 돌릴 것. (현재 총 949곳)
- **선생님(`t/`)·센터(`c/`) 페이지를 추가·삭제하면 `tools/build_sitemap.ps1` 을 다시 실행**해 `sitemap.xml` 을 갱신한다.
...
- 메뉴를 바꾸면 `index/status/teachers/centers` 4개 + **`t/` 750개 + `c/` 195개**를 전부 고쳐야 한다.
...
- **데일리 콘텐츠(블로그·RSS)는 아직 없다.** 예약작업도 없고, 커밋은 사람이 손댈 때만 나간다.
  콘텐츠 축은 **지역 + 학교명 + 과목과외**로 정했다(2026-09-10, 사용자 결정).
  - 티칭코칭(perfectedu)이 같은 축을 쓰고 있다는 점은 전달했고, 그래도 이 축으로 간다는 결정이다.
  - 겹침을 줄이려면: 티칭코칭은 `{구} {학교명} {과목}과외`로 **구 단위**이고 센터가 없다.
    우리는 센터가 있는 **동 단위**로 내려가고 "학습센터 등원"이라는 선택지가 하나 더 있다.
  - 착지점은 이미 깔려 있다 — `c/*.html` 195개(지역×학교×과목). 새 글은 여기로 앵커를 보낸다.
